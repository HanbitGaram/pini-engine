...
def PROCESS(_arg,**sub):
	...
	_arg = [arg.encode(fse) if isinstance(arg,str) else arg for arg in _arg]
...
class ExportAndroidPermissionWindow(ModalWindow):
	...
	def GUI(self):
		...
		self.Layout.label("<b>권한 설정</b>")
		self.Layout.hline();
		self.Layout.gap(3)

		with Layout.VBox(5):
			for p in self.parent.permissionList:
				self.permissionBoxes[p] = self.Layout.checkbox(str(p), permissions[p], None)
...
class ExportAndroidWindow(ModalWindow):
	...
	def __init__(self,src=None,parent=None):
		...
		self.setWindowTitle("안드로이드 익스포트")
		self.nextStep()
	...
	def nextStep(self):
		if self.work_step == 0 : 
			...
			if os.path.isdir(self.savePath.text()) == False :
				QMessageBox.warning(self,"Pini","선택한 저장 위치가 존재하지 않거나 폴더가 아닙니다.")
				return 
			if len(self.saveGameName.text()) <= 0 : 
				QMessageBox.warning(self,"Pini","게임명을 반드시 기제해주셔야합니다.")
				return 
			if len(self.savePackageName.text()) <= 0 : 
				QMessageBox.warning(self,"Pini","패키지명을 반드시 기제해주셔야합니다.")
				return 
			if self.savePackageName.text().find(" ") >= 0 : 
				QMessageBox.warning(self,"Pini","패키지명에 공백이 들어갈 수 없습니다.")
				return 
			if len(self.savePassword.text()) < 8 : 
				QMessageBox.warning(self,"Pini","패스워드는 8자 이상이여야합니다.")
				return 
			match = re.match("[a-zA-Z0-9]*",self.savePassword.text())
			if match == None or len(match.group()) != len(self.savePassword.text()) : 
				QMessageBox.warning(self,"Pini","패스워드는 영어 혹은 숫자로만 입력되야합니다.")
				return 

			match = re.match(r"[0-9\.]+",self.saveVersion.text())
			if match == None or len(self.saveVersion.text()) != len(match.group()) : 
				QMessageBox.warning(self,"Pini","버전은 숫자와 .으로만 구성되어야합니다.")
				return
	...
	def export_apk(self):
		...
		def step1():
			self.progressText.setText("키스토어 생성")
			...
			if os.path.isdir(projKeystorePath) == False :
				...
				QMessageBox.warning(self,"Pini","키스토어를 생성합니다.\n"
		...
		def step2():
			self.progressText.setText("임시 파일 삭제")
			try:
		...
		def step3(): 
			### decompile!
			self.progressText.setText("APK 수정 중")
		...
		def step4():
			### replace xml 
			try:
				self.progressText.setText("XML 수정")
				...
				print(e)
				QMessageBox.warning(self,"Pini","XML 수정에 실패하였습니다.\n"+str(e))
				return False

		def step5():
			self.progressText.setText("리소스 변경")
			### resource move!
			...
			try:
				...
				print(e)
				QMessageBox.warning(self,"Pini","리소스 변경에 실패하였습니다.\n"+str(e))
				return False

			if self.isEncryptionEnable == 1:
				self.progressText.setText("암호화 중")
				print("ENCRYPT START")
				# 이미지와 사운드 폴더만 압축합니다
				...
				try:
					...
					print(e)
					QMessageBox.warning(self,"Pini","암호화에 실패하였습니다.\n")
					return False
				print("ENCRYPT END")
			elif self.isEncryptionEnable == 2:
				self.progressText.setText("확장파일 생성 중")
				...
				try:
					...
					print(e)
					QMessageBox.warning(self,"Pini","확장파일 생성에 실패하였습니다.\n")
					return False
				print("EXTENSION END")

		def step6():
			self.progressText.setText("컴파일 중")
			...
			path = PJOIN(apkpath,"android_publish.apk")
		...
		def step7():
			self.progressText.setText("서명 인증 중 ")
			PROJPATH = inst.path
			keystorePath = PJOIN(PROJPATH,"keystore")
			keystore = PJOIN(PROJPATH,"keystore",".keystore")
		
			path1 = PJOIN(apkpath,"android_publish.apk")
			path2 = PJOIN(apkpath,"android_publish.signed.apk")
			...
			if out : 
				if out.find("password was incorrect") >= 0 :
					QMessageBox.warning(self,"Pini","비밀번호가 틀렸습니다.\n최초 입력한 비밀번호를 다시 입력해주세요.")
					return False
				if out.find("jarsigner: Certificate chain not found for:") >= 0:
					QMessageBox.warning(self,"Pini","APK 서명이 실패하였습니다. 패키지명이 키스토어를 만들 때와 다릅니다.");
					return False
				if out.find("jarsigner error: java.lang.RuntimeException: keystore load:") >= 0:
					QMessageBox.warning(self,"Pini","APK 서명이 실패하였습니다. keystore 폴더에 .keystore 파일이 존재하지 않습니다.");
					return False

			else:
				return False
		def step8():
			self.progressText.setText("임시파일 삭제")
			try:
				shutil.rmtree("resource/android/tmp")
				path1 = PJOIN(apkpath,"android_publish.apk")
				path2 = PJOIN(apkpath,"android_publish_ziped.apk")
				...
				targetFileName = ""
				for c in game:
					if re.match("[a-zA-Z0-9ㄱ-ㅎㅏ-ㅣ가-힣]", c):
						targetFileName = targetFileName + c
					else:
						targetFileName = targetFileName + "_"

				if os.path.isfile(path2) : 
					path3 = PJOIN(apkpath,targetFileName+".apk")
					count = 1
					while 1 :
						try:
							try:
							...
							if count > 200:
								...
								path3 = PJOIN(apkpath,"pini_export("+str(count-100)+")"+".apk")
							else:
								path3 = PJOIN(apkpath,targetFileName+"("+str(count)+")"+".apk")
							count += 1

					if count > 1:
						QMessageBox.warning(self,"Pini","지정한 경로에 파일을 저장하지 못하여 '"+path3+"' 경로에 apk를 저장했습니다.");
				else:
					QMessageBox.warning(self,"Pini","APK 서명이 실패하였습니다. 이전에 서명한 keystore파일이 맞는지 확인해주시기바랍니다.");
					return False
			except Exception as e:
				print(e)
				QMessageBox.warning(self,"Pini","알 수 없는 에러가 발생했습니다.\n"+str(e))
				return False
		...
		def onStep():
			if self.step >= len(self.steps):
				self.progressText.setText("APK 생성 완료")
	...
	def check_dev_dep(self):
		...
		def processing():
			if self.NeedJREInstall :
				QMessageBox.warning(self,"Pini","JRE 가 설치되어있지 않거나 JRE 1.8버전이 깔려있지 않습니다.\n깔려있는 JRE 을 모두 지우신 뒤, 1.8 버전을 설치하세요.\nJRE다운로드 페이지를 연결합니다.")
			...
			if self.TryJDKInstall : 
				if self.FindJDK() == None : 
					QMessageBox.warning(self,"Pini","JDK 자동설치에 실패하였습니다.\n직접 JDK를 설치하여 경로를 설정해주세요.\nJDK다운로드 페이지를 연결합니다.")
			...
			if self.TryADTInstall : 
				if self.FindADT() == None  :
					QMessageBox.warning(self,"Pini","ADT 자동설치에 실패하였습니다.\n직접 ADT를 설치하여 경로를 설정해주세요.\nADT다운로드 페이지를 연결합니다.")
		...
		def install_ADT():
			def ADT_INSTALL():
				...
				self.progressText.setText("ADT 압축 해제 중. (1-2분의 시간이 소요 됩니다. 잠시만 기다려주세요.)")
				...
				def ADT_FIN():
					...
					self.progressText.setText("ADT 설치 완료")
				...
				def adt_update():
					self.progressText.setText("ADT 업데이트 중. (4-5분의 시간이 소요 됩니다. 잠시만 기다려주세요.)")
					def _u():
			...
			def ADT_DOWNLOAD_PROCESSING(current,total,parentage,fin):
				self.progressText.setText("ADT 다운로드 중 ["+str(current)+"/"+str(total)+"]")
				self.progress.setValue(parentage)
				if fin : 
					ADT_INSTALL()

			self.progressText.setText("ADT 다운로드 중 []")
		...
		def install_JDK():
			def JDK_INSTALL():
				def JDK_FIN_INSTALL():
					self.progressText.setText("JDK 설치 완료")
					self.installThread = None
					self.indicator.close()
					self.indicator = None
				...
				self.progressText.setText("JDK 설치 중 (3-4분의 시간이 소요 됩니다. 잠시만 기다려주세요.)")
			...
			def JDK_DOWNLOAD_PROCESSING(current,total,parentage,fin):
				self.progressText.setText("JDK 다운로드 중 ["+str(current)+"/"+str(total)+"]")
				self.progress.setValue(parentage)
				if fin : 
					JDK_INSTALL()

			self.progressText.setText("JDK 다운로드 중 []")
	...
	def btn_find_adt(self):
		path = QtWidgets.QFileDialog.getExistingDirectory(parent=self,caption="Select ADT" )
		if path : 
			#for window!
			if self.checkADTPath(path) :
				self.inA.setText(path)
				QMessageBox.information(self,"Pini","'안드로이드 개발자 툴'(ADT)을 정상적으로 검색하였습니다.")
				Settings()["ADT_PATH"] = path
			else:
				QMessageBox.warning(self,"Pini","선택한 경로에는 '안드로이드 개발자 툴'(ADT)가 없습니다.")

	def btn_find_jdk(self):
		path = QtWidgets.QFileDialog.getExistingDirectory(parent=self,caption="Select JDK" )
		if path : 
			#for window!
			if self.checkJDKPath(path) :
				self.inJ.setText(path)
				QMessageBox.information(self,"Pini","'자바 개발자 킷'(JDK)을 정상적으로 검색하였습니다.")
				Settings()["JDK_PATH"] = path
			else:
				QMessageBox.warning(self,"Pini","선택한 경로에는 '자바 개발자 킷'(JDK)가 없습니다.")

	def btn_find_save_apk(self):
		path = QtWidgets.QFileDialog.getExistingDirectory(parent=self,caption="Select APK Save Directory" )
		if path : 
			inst = ProjectController()
			with Settings("APK_EXPORT") :
				with Settings(inst.path) :
	...
	def step1_GUI(self):
		...
		path = self.FindADT()
		path = path if path else "자동 설치"

		with Layout.HBox():
			self.Layout.label("ADT 위치")
			self.inA = self.Layout.input(path,None)
			self.Layout.button("...",self.btn_find_adt).setFixedHeight(20)
		...
		self.Layout.gap(3)
		with Layout.HBox():
			self.Layout.label("JDK 위치")
			self.inJ = self.Layout.input(path,None)
			self.Layout.button("...",self.btn_find_jdk).setFixedHeight(20)

			self.inJ.setReadOnly(True)

		self.Layout.gap(3)
		self.nextbtn = self.Layout.button("다음",self.nextStep)#.setFixedHeight(20);

	def step2_GUI(self):
		self.Layout.label("<b>2. 개발툴 다운로드</b>")
		self.Layout.hline();
		self.Layout.gap(3)

		self.progressText = self.Layout.label("설치 설명.")
	...
	def step3_GUI(self):
		...
		self.Layout.label("<b>3. apk 정보 설정</b>")
		self.Layout.hline();
		self.Layout.gap(3)
		...
		with Layout.HBox():
			with Layout.VBox():
				with Layout.HBox():
					self.Layout.label("저장위치").setFixedWidth(70)
					self.savePath = self.Layout.input(savepath,None)
					self.Layout.button("...",self.btn_find_save_apk).setFixedHeight(20);
				with Layout.HBox():
					self.Layout.label("게임명").setFixedWidth(70)
					self.saveGameName = self.Layout.input(gamename,None)
				with Layout.HBox():
					self.Layout.label("패키지명").setFixedWidth(70)
					self.savePackageName = self.Layout.input(packagename,None)
				with Layout.HBox():
					self.Layout.label("앱 버전").setFixedWidth(70)
					self.saveVersion = self.Layout.input(version,None)
				with Layout.HBox():
					self.Layout.label("비밀번호").setFixedWidth(70)
					self.savePassword = self.Layout.input(password,None)
				with Layout.HBox():
					self.Layout.button("권한 설정",self.btn_change_permission).setFixedHeight(20);
			...
			with Layout.VBox():
				...
				self.isEncryptionEnable = self.Layout.combo(["빌드옵션없음","리소스암호화","확장파일사용"])#self.Layout.checkbox(u"리소스 암호화", encryptionEnable, None)
				if encryptionEnable:
		...
		with Layout.HBox():
			self.runbtn = self.Layout.button("빌드 후 실행",self.doReserveTestRun)
			self.nextbtn = self.Layout.button("다음",self.nextStep)#.setFixedHeight(20);

	def step4_GUI(self):
		self.Layout.label("<b>4. APK 작성</b>")
		self.Layout.hline();
		self.Layout.gap(3)
		...
		self.nextbtn = self.Layout.button("종료",self.close)#.setFixedHeight(20);
